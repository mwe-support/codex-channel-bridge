...
Outbound Text 使用现有 `ChannelTextDelivery` Contract。没有 Provider Message ID 的 Response 和所有抛出的 Send Error 都被视为 Ambiguous，因为 Web Protocol 没有向 Bridge 提供 Idempotent Send Key 或确定 Reconciliation Lookup。Retry 因此保留已经披露的小重复窗口。
...
- Staged Pairing Transaction 已实现，但尚未连接 Authenticated Host-local Control Plane 和 Single-adapter Restart。Repository Acceptance Test 不会配对真实 WhatsApp Account。
- Logout 与 Revoke 尚未连接 Host-local Control Plane。
- 当前实现 Text。Media Decryption、有界 Content-addressed Mirroring、Quoted Reply 与 Send Acceptance 之外的 Receipt 属于后续阶段。
