...
新数据库使用 Bridge Schema Version 11。旧 Database 会以 Profile Reason `migration_required` 失败关闭；正常 Service Startup 不会修改它们。Host-local [`migrations.md`](migrations.md) Workflow 通过 Snapshot Evidence、完整 Plan Confirmation、事务化 Backfill/Rebuild、验证和不含内容的 Audit Record，显式支持 Schema 3 至 10→11。
...
Schema 9 增加 Archive Attachment Metadata，Schema 10 增加 QQ 原生回复流关联，
Schema 11 为 Outbox 增加不可变输出文件 Metadata。
