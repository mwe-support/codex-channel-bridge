...
- 新的空数据库初始化为 Bridge Schema Version 7。
- 未知或更旧的 Schema 返回 `migration_required`；正常启动不执行 Migration，受影响的 Profile 不启动 Codex。显式 Host-local Migration Workflow 当前只支持已知的 Version 3、4、5 或 6→7；参见 [`migrations.md`](migrations.md)。
