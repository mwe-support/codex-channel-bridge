...
| 已在当前 `main` 实现，但 rc.4 不包含 | 校验 Release Checksum 的原生安装/升级脚本；生成规范配置的快速/完全交互式设置；本地 POSIX Installer 与 macOS Setup 验收。 |
...
| 已规划或未验证 | 仅 Loopback Dashboard；Installer、Setup、Service 与 Named-pipe ACL 的真实 Windows 执行；稳定版本。 |
