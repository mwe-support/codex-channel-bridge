...
1. 由 Service Identity 安装 Node.js 22 或更高版本，以及 Codex CLI `0.149.1`。
...
2. 把本仓库复制到 `/opt/codex-channel-bridge`，执行 `npm ci` 和
   `npm run build`。普通 Service Start 绝不运行 npm。
3. 创建 `/etc/codex-channel-bridge/config.yaml` 和所有已配置的 Profile
   Directory。State 与 Secret Directory 必须通过 [`configuration.md`](configuration.md)
   中的 Owner-only Check。
4. 启用 Service 前执行 `bridge config check`、`npm run test:contract` 和
