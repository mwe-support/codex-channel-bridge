...
- Profile Store 当前实现持久化、Event-loop 外 Storage Worker、FTS5 词法检索基础、Atomic Logical Result Commit 与 Durable Outbox State Transition。完整本地 Hybrid Retrieval、Archive MCP Server、Archive Purge 和 Media Persistence 尚未实现；显式 Migration 当前只支持已知的 Schema 3、4 或 5→6，其他 Version Span 继续失败关闭。
