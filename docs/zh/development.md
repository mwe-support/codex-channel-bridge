...
- 崩溃的 Worker 会在 60 秒窗口内以 1、2、5 秒的有界延迟在 Profile 本地重启。再次崩溃会打开 Profile-local Stop Condition `worker_restart_exhausted`；Supervisor 与 Sibling Profile 保持 Live。30 秒 Cooldown 后会重置有界预算并尝试一个新的 Worker Generation；显式 Administrator Reset 尚未暴露。
