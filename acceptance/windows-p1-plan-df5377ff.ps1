...
$runtimeCommit = 'df5377ff7f2285bb50324d5118f53ab22cd220ac'
...
try {
    ...
    Write-NewJson (Join-Path $testRoot 'step-install.json') @{ installedStopped = $true; sourceCommit = $runtimeCommit }
    $phase = 'start_ready'
    [void](Invoke-BridgeJson @('service', 'start', '--name', $serviceName))
    Wait-Ready
    Write-NewJson (Join-Path $testRoot 'step-start-ready.json') @{ serviceRunning = $true; supervisorLive = $true; profileReady = $true }
    $processes = @(Get-TestProcessSnapshot)
    ...
    Write-NewJson (Join-Path $testRoot 'step-stop.json') @{ serviceStopped = $true; originalProcessTreeExited = $true }
    $phase = 'restart_ready'
    [void](Assert-OwnedRegistration)
    [void](Invoke-BridgeJson @('service', 'restart', '--name', $serviceName))
    Wait-Ready
    Write-NewJson (Join-Path $testRoot 'step-restart-ready.json') @{ serviceRunning = $true; supervisorLive = $true; profileReady = $true }
    $phase = 'uninstall'
    Remove-OwnedRegistration
    Write-NewJson (Join-Path $testRoot 'step-uninstall.json') @{ registrationRemoved = $true; processTreeExited = $true; testDataPreserved = $true }
