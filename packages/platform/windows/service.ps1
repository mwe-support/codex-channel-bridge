...
try {
  ...
  if ($Action -eq 'install') {
    ...
    if ($nativeExitCode -ne 0) { throw 'service_recovery_configuration_failed' }
  ...
  if ($Action -eq 'uninstall') {
    ...
    $nativeExitCode = $LASTEXITCODE
    if ($nativeExitCode -ne 0) { throw 'service_delete_failed' }
  }
  ...
  if ($reason -eq 'windows_service_operation_failed') {
    $kind = 'hresult'
    $code = $_.Exception.HResult
    for ($cause = $_.Exception; $null -ne $cause; $cause = $cause.InnerException) {
      if ($cause -is [ComponentModel.Win32Exception]) { $kind = 'win32'; $code = $cause.NativeErrorCode; break }
    }
    if ($null -ne $nativeExitCode) { $kind = 'exit'; $code = $nativeExitCode }
    $reason = '{0}_{1}_{2}_{3}' -f $reason, $stage, $kind, $code
  }
